...
var currentRelay AutoGenerated

func RelayFilter(client Client, relays AutoGenerated) FilteredRelays {
	// locations := []string{
	...
	for i := 0; i < len(countryList); i++ {
		...
		for x := 0; x < len(relays.Wireguard.Relays); x++ {
			...
			// 		do thing
			if slices.Contains(countryList, relays.Wireguard.Relays[x].Location) {
				...
				// fmt.Println("Match")
				// fmt.Println(relays.Wireguard.PortRanges)
				// fmt.Println("X: ", x)
				// fmt.Println("Location: ", relays.Wireguard.Relays[x].Location)
				// fmt.Println("Pubkey: ", relays.Wireguard.Relays[x].PublicKey)
				// fmt.Println("Hostname: ", relays.Wireguard.Relays[x].Hostname)
				// fmt.Println("Relay IPv4 in: ", relays.Wireguard.Relays[x].Ipv4AddrIn)
				// fmt.Println("")
			} else {
				continue
			}
